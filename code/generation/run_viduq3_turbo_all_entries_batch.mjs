...
const OUTPUT_ROOT = path.join(__dirname, 'outputs', 'viduq3_turbo_720p_5s_all_entries');
...
function parseArgs(argv) {
  const args = {
    ...
    entriesPath: process.env.VIDU_ENTRIES_PATH?.trim() || DEFAULT_ENTRIES_PATH,
    ...
    context = buildContext(
      {
        ...
        audio: resumeState.config.audio,
        ...
        batchSize: resumeState.config.configured_limits?.batch_size ?? args.batchSize,
        size: resumeState.config.size ?? args.size,
      },
...
main().catch((error) => {
  console.error('\nViduQ3-Turbo all-entries batch run failed.');
